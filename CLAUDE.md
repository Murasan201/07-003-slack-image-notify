...
- **コメント規約**:
  ...
  - **詳細は `COMMENT_STYLE_GUIDE.md` に従う**
- **コーディングガイドライン**:
  - **詳細は `python_coding_guidelines.md` に従う**
  - PEP 8準拠で命名規則（snake_case、PascalCase）に統一
  - すべての実行可能なPythonファイルに `if __name__ == "__main__":` を含める
  - すべての関数・クラスにdocstringを記述
  - try-finallyでリソースのクリーンアップを確実に実行
- **関数分割**: 機能ごとに適切に分割
- **エラーハンドリング**: 必須実装
